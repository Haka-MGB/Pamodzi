...
import { updateTenant, deleteTenant, verifyUserPassword } from '@/lib/server/db'
import { fail, ok, readJson, requireSameOrigin, requireUserId } from '@/lib/server/http'
import { parseTenantUpdates } from '@/lib/server/validation'
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const input = parseTenantUpdates(await readJson(request))
  if (!input) return fail('Check the tenant details.')
  const id = (await params).id
  const updated = await updateTenant(userId, id, input)
  if (!updated) return fail('Tenant not found.', 404)
  return ok(updated)
}
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const removed = await deleteTenant(userId, id)
  if (!removed) return fail('Tenant not found.', 404)
  return ok(removed)
}
